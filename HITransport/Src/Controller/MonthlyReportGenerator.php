...
if (isset($_POST['Submit'])) { 
	...
	if($status == 1){
		$getApproved = "SELECT * FROM booking WHERE booking_date LIKE '".$start."%' AND status='1' AND department='".$department."'";
		$resultApproved = mysqli_query($con, $getApproved);	
		if (mysqli_num_rows($resultApproved)!=0) {
			$pdf->AddPage('L', 'A4');
			$html = '<div>
					<h4><u>APPROVED BOOKING DETAILS</u></h4>
				</div>
			...
			header('Location:../MonthlyReport.php?message=2');		
		}
	} else {
		$getRejected = "SELECT * FROM booking WHERE booking_date LIKE '".$start."%' AND status='2' AND department='".$department."'";
		$resultRejected = mysqli_query($con, $getRejected);
		if (mysqli_num_rows($resultRejected)!=0) {
			$pdf->AddPage('L', 'A4');
			$html = '<div>
					<h4><u>REJECTED BOOKING DETAILS</u></h4>
				</div>
				<table width="95%" align="center"  style="border:#D22929 solid 2px;padding:10px;" border="0">
				  <tr>
					<th bgcolor="#D22929" scope="col"><span class="style10"><b>EPF No</b> </span></th>
	...
	header('Location:../MonthlyReport.php?message=1');
}
?>