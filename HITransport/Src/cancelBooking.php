...
<table width="100%" align="center"  style="border:#D22929 solid 2px;padding:10px;" border="0">
  <tr>
    ...
    <th bgcolor="#D22929" scope="col"><span class="style10">Cancel Booking</span></th>
  </tr>
<?php
$get= "SELECT * FROM staff_transport WHERE status='0' and email='".$_SESSION['email']."'";
	...
	while($row= mysqli_fetch_array($result)){
		...
		$id = $row['id'];
			echo '<tr style="height:30px; padding:4px;">
					...
					<form action="controller/cancelStaffController.php" method="post">
					<div align="center"><input hidden type="text" name="id" id="id" value="'.$id.'"/></div>
					<td><div align="center"><input type="submit" value="Cancel"/></div></td>
					</form>
				</tr>';	
	}
} else {
...
<?php
$get= "SELECT * FROM booking WHERE status='0' and email='".$_SESSION['email']."'";
$result= mysqli_query($con, $get);
if(mysqli_num_rows($result)!=0){
	while($row= mysqli_fetch_array($result)){
		$epf = $row['epf_no'];
		...
		$id = $row['id'];
			echo '<tr style="height:30px; padding:4px;">
					...
					<td><div align="center">'.$type.'</div></td>
					<form action="controller/cancelBookingController.php" method="post">
					<div align="center"><input hidden type="text" name="id" id="id" value="'.$id.'"/></div>
					<td><div align="center"><input type="submit" value="Cancel"/></div></td>
					</form>
				</tr>';	
...
<?php
$get= "SELECT * FROM goods_delivery WHERE status='0' and email='".$_SESSION['email']."'";
$result= mysqli_query($con, $get);
if(mysqli_num_rows($result)!=0){
	while($row= mysqli_fetch_array($result)){
		$epf = $row['epf_no'];
		...
		$id = $row['id'];
			echo '<tr style="height:30px; padding:4px;">
					...
					<td><div align="center">'.$date.'</div></td>
					<td><div align="center">'.$reason.'</div></td>
					<form action="controller/cancelDeliveryController.php" method="post">
					<div align="center"><input hidden type="text" name="id" id="id" value="'.$id.'"/></div>
					<td><div align="center"><input type="submit" value="Cancel"/></div></td>
					</form>
				</tr>';	
